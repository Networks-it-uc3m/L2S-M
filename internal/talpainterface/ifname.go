...
func GetIfName(nodeName, providerName string, index int) string {
	...
	ifid := dp.NewIfId(dp.GetSwitchName(dp.DatapathParams{NodeName: nodeName, ProviderName: providerName}))
	return ifid.Port(index)
}
func ProbeInterface(node, provider string) string {

	ifid := dp.NewIfId(dp.GetSwitchName(dp.DatapathParams{NodeName: node, ProviderName: provider}))
