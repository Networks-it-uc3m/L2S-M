...
func main() {
	...
	if secureMetrics {
		// Protect metrics using Kubernetes authn/authz without kube-rbac-proxy.
	...
	if err := (&controller.QuarantinePodRequestReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "QuarantinePodRequest")
		os.Exit(1)
	}
	if err := (&controller.CcipsOverlayReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "Failed to create controller", "controller", "CcipsOverlay")
		os.Exit(1)
	}
